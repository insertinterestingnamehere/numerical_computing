...
The order in which you enter in your argument values when calling your function should correspond to the order in which the parameters were defined. One way around this is to use \li{keyword=value} pairs in your function declaration. The value you give to a parameter when defining the function is the default value.
...
>>> def function_name(a, b="Default value: string, number, etc.", c=2):
...     print a
...     print b
...     print c
...     

# Call function and with a = 1
>>> function_name(1)
1
Default value: string, number, etc.
2

# Define a and redefine c
>>> function_name(1, c=3)
1
Default value: string, number, etc.
3

# When using keyword=value, order doesn't matter
>>> function_name(c=3, a=4)
4
Default value: string, number, etc.
3
>>> function_name(4, 5, 6)
...
Non-keyword parameters must be defined before keyword arguments general pattern for defining parameters when writing and calling a function must start with defining non-keyword parameters first (`a' in the last example) and then keyword arguments, if any.
...
...     print kwargs.keys()
...     
>>> function_name("Hello", 2, 1, apples = 3, oranges = 2)
First show args
Hello
2
1
