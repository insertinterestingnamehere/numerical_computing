...
data-analytic abstractions present in other popular data software, such as R and Stata.
...
We will explore the first two data structures in some detail, leaving the \li{Panel} (the least-used
pandas data structure) for the online documentation. 
...
\li{read_csv} function in more detail. To read other types of file formats, refer to the online pandas documentation.
To read a csv data file into a \li{DataFrame}, simply call the \li{read_csv} function with the path to the csv file,
along with the appropriate keyword arguments. Below we list some of the most important:
\begin{itemize}
\item \li{delimiter}:
...
a histogram for each column in a \li{DataFrame}, with the result shown in Figure \ref{pandas:histogram}.
