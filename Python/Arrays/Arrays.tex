...
NumPy arrays are homogeneous meaning that they only store elements of the same data type, however, that data type can be any arbitrary object.
...
\item \li{np.empty_like}: Allocates a new uninitialized array with the same shape as the input array.
\item \li{np.ones}: Allocates and array and initializes each element to $1$.
\item \li{np.ones_like}: Allocates a new array of ones with the same shape as the input array.
\item \li{np.zeros}: Allocates and array and initializes each element to $0$.
\item \li{np.zeros_like}: Allocates a new array of zeros with the same shape as the input array.
...
Understand that even though they may look the same, views and copies very different.
