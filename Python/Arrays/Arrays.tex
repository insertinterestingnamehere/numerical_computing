...
Indexing for a 1-D NumPy array works exactly like indexing for a Python list. To access a single entry of a multi-dimenional array, say a 3-D array, you should use the syntax \li{f[i, j, k]}. While the syntax \li{f[i][j][k]} will also work, it is significantly slower because each bracket returns an array slice. Similarly, slicing an array works just like slicing a list, but with more dimensions.
\begin{lstlisting}
...
>>> amask = np.logical_and(a<23, a>15); bmask 
...
\begin{lstlisting}
# Return an array of elements b[0,0], b[0, 0], b[2,3], and b[4,1].
>>> a[[0, 0, 2, 4], [0, 0, 3, 1]] 
array([ 0,  0, 13, 21])
...
# Change the third element of m to 500
# Changing m also changes k.
>>> m[2] = 500 
>>> m
array([  0,   1, 500,   3,   4])
>>> k 
...
# Change the third element of n to 500
# Changing n does not affect j.
>>> n[2] = 500 
>>> n
array([  0,   1, 500,   3,   4])
>>> j 
...
\begin{problem}\label{numpy_is_fast_problem}
...
A matrix in NumPy is just a 2-D array. On the other hand, a matrix in Python can be implemented as a list of lists. You can create a $k \times k$ matrix  of the integers 0 through $k^2-1$ as follows.
...
For \li{k=100}, \li{k=200}, and \li{k=300}, time\footnote{In IPython, you can find the execution time of a line of code by prefacing it with \li{\%timeit}. If you aren't using IPython, you will need
to use the timeit function documented here: \url{https://docs.python.org/2/library/timeit.html}.} how long it takes to square a $k \times k$ matrix using the method \li{arr_mult} defined above and the NumPy method for matrix multiplication.
