...
The reason for the drastic speed diference is that Python, as a high level interpreted language, tends to be slower than lower level compiled languages.
...
These arrays are commonly known as \li{ndarrays}.
...
For those who are familiar with object oriented programming, it is worth noting that many of the functions we discuss are also implemented as methods of \li{ndarray} objecs.
For this lab, you should know that objects have attributes that can be accesed using the \li{.} operator as you will see below.
...
Most of these functions also allow you to specify an array for the output, which allows you to do operations without creating new arrays.
For example
\begin{lstlisting}
...
For example:
\begin{lstlisting}
: np.max(A, axis=0) #max along horizontal axis
\end{lstlisting}
...
The NumPy \li{transpose} funciton will also do this.
...
Start with it set equal to the tolerance.
\item While the difference is less than or equal to the tolarance
\begin{itemize}
