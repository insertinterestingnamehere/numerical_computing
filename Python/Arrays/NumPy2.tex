...
def egcd(a, b):
	# this is an Extended Euclidean Algorithm function
	x, y, u, v = 0, 1, 1, 0
	while a != 0:
...
def modinv (a, m):
	...
	if g != 1:
		return None
		# indicating the modular inverse does not exist
	else:
		return x % m
	
# for more information, visit http://en.wikibooks.org/wiki/Algorithm_Implementation/Mathematics/Extended_Euclidean_algorithm

def blockize(msg, n):
	# split a message into n-length blocks
	# this will also map the letters to the numbers needed for the Hill cipher
	...
	# return a list of arrays, each of length n.
	# do the lookups needed to translate each character to a number 
	# ("x" is a fill character)
	
	return list(map(np.array, grouper(map(lut._getitem_, msg), n, fillvalue = lut["x"])))
...
def getText(msg):
	...
	for i in msg:
		text.append("".join(map(ilut._getitem_,i)))
	return "".join(text)
	
def getMatrix(n):
	'''
	Return an nxn integer matrix that is invertible mod 26. Do this by repeatedly creating a random nxn integer matrix, reducing it mod 26, and then checking to see that it is invertible by finding the GCD of the rounded determinant and 26. If this GCD is equal to 1, break out of the loop and return the matrix. Note, when you use the gcd function, it MUST take integers, so be sure to cast the determinant to an int after rounding it.
	'''
...
def encode(msg, k):
	...
	# msg is a list of arrays, as in the output of blockize
	# k is a matrix
...
def arcsin_approx():
	# create a polynomial object up to the n=70 term, approximating arcsin.
	# return your approximation of pi
	
def W_approx():
	# create a polynomial object up to the n=20 term, approximating the lambert W function.
	# return your approximate value of x such that x*exp(x) = .25
