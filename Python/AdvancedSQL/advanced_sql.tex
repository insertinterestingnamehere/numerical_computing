...
Another example would be a driver's license belongs which to only one person.
...
For the rest of the lab, we will be using the follwing tables: \ref{table:students}, \ref{table:majors}, \ref{table:grades}, and \ref{table:classes}.
...
There are several keywords that can make a \lsql{SELECT} statement very powerful.
...
For example, \lsql{SELECT name, studentid FROM students WHERE studentid LIKE '\%4'} will return all the students that have a student ID that ends with the digit 4.
