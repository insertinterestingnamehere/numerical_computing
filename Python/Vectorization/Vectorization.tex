...
\item For an array \li{P} of shape \li{n} and a boolean array of shape \li{D} compute the array \li{A} such that \li{A[i]} is $\sum_{P[i,j] = True} \left( P[i] - P[j]\right)$.
...
\item For an array representing a color image calculate the difference between each color value and the average value for each pixel.
