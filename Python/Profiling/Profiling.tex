...
This code is then compiled into C, which---depending on the situation---can run much faster than the Python equivalent.
...
\footnote{Sometimes you will see speedup after Cythonizing a Python function.}
...
NumPy's array methods are faster than a Cython equivalent you could code yourself.
You should use existing functions whenever possible.
...
\end{itemize}
Your solution should include a before and an after profiling showing the overall optimization in each of your solutions.  
It should also include a list of changes, the reasoning behind the changes, and the effect of the changes on runtime.  

If there are no reasonable gains to be made in performance, please note that as well. 
It is often more important for code to be readable than to execute quickly.
\end{problem}
...
For more details, see \url{http://docs.cython.org/src/tutorial/cython_tutorial.html}.
