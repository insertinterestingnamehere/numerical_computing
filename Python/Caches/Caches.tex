...
To compile the included C program, \li{cache_size.c}, you will need a C compiler.
We recommend gcc with the flags \texttt{-O3 -mtune=native}.
...
Python has a library, \li{numexpr}, which is designed to utilize the CPU cache as well as avoid unnecessary temporary arrays.
...
On the authors machine, \li{numexpr} executed the factored and unfactored expressions about 25 times faster than the original function.
\begin{lstlisting}
import numexpr as ne
yne = ne.evaluate('1.25*x**3 - .33*x**2 + .75*x + 1.2')
\end{lstlisting}
Using \li{numexpr} instead of NumPy can speed up simple vector computations significantly.
