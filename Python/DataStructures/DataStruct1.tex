...
In the example below, we will use Python's built-in \li{hash} function.
...
Initialize a hash table of size 4 using the example \li{HashFunction()} given below.
\begin{lstlisting}
def HashFunction(string, tablesize, prime=5417):
    index = (len(string) * prime)  %  Tablesize
    return index
\end{lstlisting}
Insert the following felines: `lion', `tiger', `cheetah', `cougar', `colocolo', `cat', `clouded leopard', and `jaguar'.
