...
Queues use a `first in, first out' implementation, meaning that the first thing put in will be the first thing taken out.
...
Python's \li{collections} module already contains a deque object, which is discussed in section
...
\emph{Trees} are similar to linked lists, but the nodes of a tree can reference with greater versatility (rather than simply referencing the \li{next} or \li{previous} node).
...
\begin{tikzpicture}[
  ...
  \node[draw=none, node distance = 4cm][below of=-1]{Min Heap};	
\end{tikzpicture}
\end{center}
...
\path[->, >=stealth, auto]
	(98) edge[bend right, red] (8)
	(8) edge[bend right, red] (53)
	(53) edge[bend right, red] (87)
	(8) edge[bend right] (98)
	(53) edge[bend right] (8)
	(87) edge[bend right] (53);
...
\tikzstyle{block} = [rectangle, draw, minimum height = .55cm, minimum width = 1.5cm]
  ...
  \tikzstyle{block} = [draw=none]
   \node[block] [above=0cm of 0]{0};
   \node[block] [above=0cm of 1]{1};
   \node[block] [above=0cm of 2]{2};
   \node[block] [above=0cm of 3]{3};
\end{tikzpicture}
\end{center}
...
\begin{tikzpicture}[->,>=stealth',shorten >=1pt, auto, thick]
 \tikzstyle{block} = [rectangle, draw, minimum height = .55cm, minimum width = 1.5cm]
   ...
   \node[block] (1) [right=0 cm of 0] {};
   \node[block] (2) [right=0 cm of 1] {};
   \node[block] (3) [right=0 cm of 2] {};
   \node[block] (4) [right=0 cm of 3] {`cat'};
   \node[block] (5) [right=0 cm of 4] {`lion'};
   \node[block] (6) [right=0 cm of 5] {};
   \node[block] (7) [right=0 cm of 6] {`leopard'};
 \tikzstyle{block} = [draw=none]
  \node[block] [above=0cm of 0]{0};
  \node[block] [above=0cm of 1]{1};
  \node[block] [above=0cm of 2]{2};
  \node[block] [above=0cm of 3]{3};
  \node[block] [above=0cm of 4]{4};
  \node[block] [above=0cm of 5]{5};
  \node[block] [above=0cm of 6]{6};
  \node[block] [above=0cm of 7]{7};
\end{tikzpicture}
\end{center}
