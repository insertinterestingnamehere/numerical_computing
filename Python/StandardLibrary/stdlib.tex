...
\begin{lstlisting}
open(filename, 'rb')    # open a file read-only in binary mode.  No newline conversion.
open(filename, 'rU')    # Universal newline support. Recognizes any newline character.
\end{lstlisting}
\end{info}
...
with open('output.txt', 'w'):
    ...
    # we can only write strings to a file
    f.write(str(num))
\end{lstlisting}
...
with open('filename.txt', 'r') as f:
        read_data = f.read()
# read_data is accessible outside of with-block
print read_data
\end{lstlisting}
...
The name of namespace is stored in the \li{__name__} attribute.
...
if __name__ == "__main__":
    ...
    print "I have imported by another python module."
    print __name__
\end{lstlisting}
...
We can define an optional alias if the module name is too long.
The names in the module are accessed by \li{module.name}.
\item \li{from module import name [as alias]} This will import a particular name in the module directly into the main namespace.
...
However convenient this form might be, it is strongly discouraged to import modules this way as it can lead to many unknown name conflicts.
\end{description}
...
\end{lstlisting}
Our main program will return the 
...
However, when we imported the functions from our module, we ended up overwriting Python's \li{min()} and \li{max()}.
...
These random numbers are not completely random, however, for most applications they are seemingly random enough.
...
\begin{problem}
Write a function that will generate a random map between two lists that are equal in length of arbitrary items.
...
You get to decide a survival threshold for all tributes (the probability of surviving the event).
