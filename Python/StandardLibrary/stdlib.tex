...
The \li{open} command accepts three arguments: filename, mode, and buffering.
...
Some of the more commonly used functions of the \li{random} are listed below.
\begin{itemize}
...
In our last code example, calling \li{next()} the first time will yield the value 1.  Calling \li{next()} again will yield 2.  Calling \li{next()} again will yield 3.  Finally, calling \li{next()} again will raise an exception signaling that the generator has finished executing and has yielded all its values.
