...
def range_gen(start, stop, step=1):
    ...
    while i < stop:
        ...
        i = i + step
\end{lstlisting}
The two functions look very similar, but they behave very differently.
The first function when executed will immediately build a list, one element at a time until finished
...
The real difference in the two functions lies in the time the function takes to execute when
...
The \li{xrange()} function that is built into Python is a generator based version of \li{range()} and is generally faster when used in \li{for} loops; you should make a habit of using \li{xrange()} instead of \li{range()}.
...
\li{izip()} & \li{izip('ABCD', 'xy') --> Ax By} (like a zipper matching elements) \\
\hline
...
We encourage you to view the documentation for \li{itertools} on the Python website, which also includes various recipes for other useful generator functions.
