...
However, when we imported the functions from our module, we ended up overwriting Python's \li{min()} and \li{max()} functions.
...
Some of the more commonly used methods of the \li{random} module are demonstrated below.
\begin{lstlisting}
import random
...
Standard tuples in Python are accessed by index while named tuples allow access via index or by  fieldname (defines the tuple elements and acts like a key).
...
g.next()        # return 2
g.next()        # return 3
# Raise a StopIteration exception; there are no more values to return.
g.next()
\end{lstlisting}
...
# Print timing for rotate method
