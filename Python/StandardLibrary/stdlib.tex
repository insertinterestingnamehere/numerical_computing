...
Instead of using f = \li{open}, it is preferable to open files using \li{with}.
...
We can also use \li{as alias} to define an optional alias if the module name is too long.
...
The \li{csv} module provides \li{reader} and \li{writer} objects. \li{DictReader} and \li{DictWriter} are analagous objects that use dictionaries for handling data.
...
\begin{info}
The \li{random} module has functions that can sample from a variety of different statistical distributions such as: uniform, normal, beta, gamma, exponential, etc. To learn more about random numbers in Python see the documentation at \url{https://docs.python.org/2/library/random.html}.
\end{info}
