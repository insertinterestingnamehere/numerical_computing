...
\section*{Simple Message Passing}
  Let us begin by demonstrating a program designed for two processes. One will draw a random number and then send it to the other. We will do this using the routines \li{Comm.Send} and \li{Comm.Recv} (short for ``recieve''):
  ...
  % TODO ? talk about how the output gets garbled. Figure out what -u flag does ("unbuffered")
...
\section*{Parallelizing the Trapezoidal Rule}
  ...
  Second, notice how the results are summed. We know how many results we should be recieving, so the root process simply accepts the messages in the order that they arrive. This is achieved using the tag \li{MPI.ANY_SOURCE} in the \li{COMM.Recv} method. In following labs we will learn about even more effective ways to gather the results of many different processes' computations.
