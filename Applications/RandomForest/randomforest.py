...
class Forest(object):
    ...
    def __init__(self, data, y, classes, variables, types, maxdepth, tol):
        ...
        self.tol = tol
        self.availablevars = np.array([True for i in xrange(self.n_variables)])
    ...
    def train(self, mtry, n_trees):
        ...
        for i in xrange(n_trees):
            print i
            self.trees.append(
                ForestNode(self.data, self.y, self.classes, self.variables,
                           self.availablevars, self.types, mtry, self.tol, 1, self.maxdepth))

    def predict(self, sample):
        predictions = [self.trees[i].predict(
            sample) for i in xrange(self.n_trees)]
...
class ForestNode(object):
    ...
    def __init__(self, data, y, classes, variables, available_vars, types, mtry, tol=.1, depth=1, maxdepth=10):
        ...
        if self.depth < self.maxdepth and self.gini > self.tol and self.n_available_vars >= mtry:
            ...
            tempavailablevars = np.copy(self.available_vars)
            for i in xrange(self.n_variables):
                if self.available_vars[i]:
                    if len(set([x[i] for x in data1])) == 1:
                        tempavailablevars[i] = False
            self.leftchild = ForestNode(
                data1, y1, classes, variables, tempavailablevars, types, mtry, tol, depth + 1, maxdepth)
            tempavailablevars = np.copy(self.available_vars)
            for i in xrange(self.n_variables):
                if self.available_vars[i]:
                    if len(set([x[i] for x in data2])) == 1:
                        tempavailablevars[i] = False
            self.rightchild = ForestNode(
                data2, y2, classes, variables, tempavailablevars, types, mtry, tol, depth + 1, maxdepth)
            self.leaf = False
        else:
    ...
    def numsplit(self, variable, value):
        inds1 = [i for i in xrange(int(
            self.n_samples)) if self.data[i][variable] <= value]
        inds2 = [i for i in xrange(int(
            self.n_samples)) if self.data[i][variable] > value]
    ...
    def optimalnumsplit(self, variable):
        possibilities = np.sort(np.array(list(set([self.data[i][variable] for i in xrange(int(self.n_samples))]))))
        ...
        if len(possibilities) <= 1:
            ...
            for i in range(1, len(possibilities)):
                tempsplitscore, tempinds1, tempinds2, tempy1, tempy2 = self.numsplit(
                    variable, possibilities[i])
    ...
    def optimalsplit(self):
        ...
        vars = np.sort(random.sample(inds, self.mtry))
