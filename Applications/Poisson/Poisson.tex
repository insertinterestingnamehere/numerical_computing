...
&= \frac{1}{h^2}(U_{i-1,\,j} + U_{i+1,\,j} + U_{i,\,j-1} + U_{i,\,j+1}-4U_{i,\,j}).
 \end{align*}
 Then the set of equations  $\triangle_h U_{ij} = f_{ij}$, $i,j = 0,\ldots,N-2$  $i,j = 1,\ldots,m$ can be written in matrix form as
 $$AU + q  = f$$
 ...
 \end{bmatrix}
 \]
 where $I$ is the $N-1\times N-1$  $m\times m$ identity matrix, and $T$ is the tridiagonal matrix
 \[
...
To make this easier, we'll make a function (\li{rho1}) to calculate the $\rho$ value  at a certian point in space based the position of an atom and it's relative charge. 
...
Using a function for $\rho$, we can solve Poisson's equation for the electric potential field.
\begin{problem}
