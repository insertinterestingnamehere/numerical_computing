...
\objective{This section teachings about shearching graphs and uses the parlor game the Six Degrees of Kevin Bacon as an application to graphs}
...
\begin{algorithm}
\begin{algorithmic}[1]
\Procedure{BFS/DFS}{$G, start, destination$}
	\State $Q \gets \text{Deque(BFS) or list(DFS) with start node in it}$	\Comment{initialization steps}
	...
	\While{$Q \text{ has elements}$}				\Comment{Go through the graph}
		\State $t \gets \text{Q's left (BFS) or right(DFS) element}$		\Comment{Words}
		\State $\text{add t to the visited list}$
		\If{$t==destination$}		\Comment{return at destination node}
			\State \pseudoli{return} $t,visited$
		
		\Else
			\For{$e \text{ the adjacent nodes of t}$}
				\If{$e \text{ not in marked}$}
