...
\[\Delta u = u_{xx}+u_{yy} \approx \frac{U_{l-1,m}^n - 2 U_{l,m}^n + U_{l+1,m}^n}{(\Delta x)^2} + \frac{U_{l,m-1}-2 U_{l,m}^n + U_{l,m+1}^n}{(\Delta y)^2}\]
...
\[\Delta u \approx (U_{l-1,m}^n - U_{l,m}) + (U_{l+1,m}^n - U_{l,m}) + (U_{l,m-1} - U_{l,m}) + (U_{l,m+1} - U_{l,m})\]
...
\[U_{l,m}^{n+1} = U_{l,m}^n + (U_{l-1,m}^n - U_{l,m}) + (U_{l+1,m}^n - U_{l,m}) + (U_{l,m-1} - U_{l,m}) + (U_{l,m+1} - U_{l,m})\]
We will now limit the diffusion near the edges of objects by making the following modification:
\begin{equation*}
\begin{split}
U_{l,m}^{n+1} =& U_{l,m}^n + \lambda (g(|U_{l-1,m}^n - U_{l,m}|)(U_{l-1,m}^n - U_{l,m}) + g(|U_{l+1,m}^n - U_{l,m}|)(U_{l+1,m}^n - U_{l,m}) \\
 &+ g(|U_{l,m-1} - U_{l,m}|)(U_{l,m-1} - U_{l,m}) + g(|U_{l,m+1} - U_{l,m}|)(U_{l,m+1} - U_{l,m}))
\end{split}
\end{equation*}
...
\begin{split}
U_{l,m}^{n+1} =& U_{l,m}^n + \lambda (g(|U_{l-1,m}^n - U_{l,m}|)(U_{l-1,m}^n - U_{l,m}) + g(|U_{l+1,m}^n - U_{l,m}|)(U_{l+1,m}^n - U_{l,m}) \\
 &+ g(|U_{l,m+1} - U_{l,m}|)(U_{l,m+1} - U_{l,m}))
\end{split}
\end{equation*}
...
\begin{equation*}
U_{l,m}^{n+1} = U_{l,m}^n + \lambda (g(|U_{l+1,m}^n - U_{l,m}|)(U_{l+1,m}^n - U_{l,m}) + g(|U_{l,m+1} - U_{l,m}|)(U_{l,m+1} - U_{l,m}))
\end{equation*}
...
This diffusion process can be carried out in the exact same way, on each of the arrays of intensities for each color, but instead of detecting edges just in one color, we need to detect edges in any color, so instead of using something of the form $g(|U_{l+1,m}^n - U_{l,m}|)$ as before, we will now use something of the form $g(||U_{l+1,m}^n - U_{l,m}||)$, where $U_{l+1,m}^n$ and $U_{l,m}$ are vectors now instead of scalars.
