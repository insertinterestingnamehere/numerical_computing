...
For this example we used $\sigma = .7$ and $\lambda = .25$
...
\begin{problem}
Write a new version of the previous problem with the following boundary conditions added:
For the top edge let 
\begin{equation*}
\begin{split}
v_{l,m}^{n+1} =& v_{l,m}^n + \frac{4 \lambda}{3} (g(|v_{l-1,m}^n - v_{l,m}|)(v_{l-1,m}^n - v_{l,m}) + g(|v_{l+1,m}^n - v_{l,m}|)(v_{l+1,m}^n - v_{l,m}) \\
...
\begin{equation*}
v_{l,m}^{n+1} = v_{l,m}^n + 2 \lambda (g(|v_{l+1,m}^n - v_{l,m}|)(v_{l+1,m}^n - v_{l,m}) + g(|v_{l,m+1} - v_{l,m}|)(v_{l,m+1} - v_{l,m}))
\end{equation*}
