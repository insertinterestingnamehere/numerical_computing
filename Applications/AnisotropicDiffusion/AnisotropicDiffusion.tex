...
When implementing this scheme for colored images, use the $2$-norm on 3-space, i.e $||x||=max(x1,x2,x3)$ where $x1$, $x2$, and $x3$ are the different coordinates of $x$.
...
Use the corresponding vector versions of the boundary conditions given in Problen \ref{prob:anisdiff_bw}.
...
The \li{None} tels NumPy to create an array with an extra dimension of length 1 wherever the \li{None} appears.
