...
\begin{problem}
Write a function called \li{discretenorm} that accepts an integer $N$ representing the number of discrete points 
...
\begin{enumerate}
   \item First we establish our approximation of $\ve$ using the \li{discretenorm} function created in Problem 1. 
   ...
   Using the function \li{discretenorm}, generate the probability distribution $\Gamma$ for $\ve$. 
   ...
   $\ve,W,W'$.  Thus $\ve u(W-W')$ will be represented by a three-dimensional array of size $N\times N\times K$.
     This can be achieved using the tile function.  For example, if $u(W-W')$ is represented by the $N\times N$
      matrix \li{util_grid}, and \li{eps} is the vector of $\ve$ values, then we could create the 
...
\end{lstlisting}
       ...
       The input \li{(1,1,K)} tells \li{sp.tile} to repeat \li{util_grid} K times in the third dimension.  
       Note we have to put \li{util_grid[:,:,sp.newaxis]} in order to have a three-dimensional input since 
       \li{util_grid} is only two dimensional.
