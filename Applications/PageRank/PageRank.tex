...
In practice, you should never construct a diagonal matrix, invert it, then do matrix multiplication.
...
These operations are mathematically equivalent versions of the inversion and multiplication, but are much more efficient.
...
Your function should accept a file name for the data and a size \li{n} that defaults to \li{None}.
The size will allow you to specify that you want to consider only the upper left $n \times n$ portion of the array.
\end{problem}
...
We won't have you implement this sort of solver here.
...
The computed value for $R$ gives the page rank associated with each webpage.
