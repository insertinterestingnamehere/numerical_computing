...
\caption{The graph of $U^{0}$, the approximation to the solution $u(x,t=0)$ for \ref{prob:heat_exercise1}.}
...
\caption{The graph of $U^{10}$, the approximation to the solution $u(x,t=.4)$ for \ref{prob:heat_exercise1}.}
\label{fig:heatexercise1b}
\end{figure}

\begin{info}
Finite difference schemes, though they can be represented using matrix multiplication, should not usually be \emph{implemented} directly using matrix multiplication.
Using NumPy, it is best to vectorize the difference scheme so that you do not have to loop over the spatial indices.
If you are using a language that has faster loops (like C, C++, Fortran, or Cython), it could work well to loop directly through the indices in both time and space.
\end{info}
...
\begin{align}
	...
	&{ } u(x,0) = 2\max\{.2 - |x-5.|,0\}.
	\end{split}\label{prob:heat_exercise1}
\end{align}
...
def sine_animation(res=100):
    ...
    # When this function is called, x and y aren't in
    # the current scope, so include them as default
    # arguments so they can be used in the body of the function.
...
\begin{align}
	...
	&{ } u(x,0) = \max\{1 - x^2,0\}
	\end{split}\label{prob:heat_exercise2}
\end{align}
...
\caption{The initial and final states for \ref{prob:heat_exercise2}.}
