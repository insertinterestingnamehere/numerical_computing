...
\begin{equation}\label{EqV}
   V(w,w',s) = \begin{cases}
               ...
               \end{cases}
\end{equation}
where employment status is a binary variable $s=\{E,U\}$ ($E$ indicates ``employed" and $U$ indicates ``unemployed"); 
a person can be either employed or unemployed.

If an individual's job status is employed ($s = E$) in a given period, then the expected utility is the utility of 
...
To calculate this expectation, we need to know the joint probability distribution over $w''$ and $s'$.
