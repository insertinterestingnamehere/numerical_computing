...
class MandlebrotDisplay(object):
    ...
    def __call__(self, xstart, xend, ystart, yend):
        ...
        self.y = sp.linspace(ystart, yend, self.height).reshape(-1,1)
        #c = self.x + 1.0j * self.y
        c = -0.8+0.125j
        threshold_time = sp.zeros((self.height, self.width))
        z = sp.zeros(threshold_time.shape, dtype='complex128')
        mask = sp.ones(threshold_time.shape, dtype='bool')
        for i in xrange(self.niter):
            z[mask] = z[mask]**self.power + c#[mask]
            mask = (sp.absolute(z) < self.radius)
            threshold_time += mask
