...
When we use SciPy in book, it will always be imported as follows. This python statementimports all the main SciPy methods for use and make them available in the \li{sp} object.
\begin{lstlisting}
import scipy as sp
\end{lstlisting}
...
The problem with \li{\%timeit} is that it is only defined inside The problem arises when trying to use \li{timeit} directly.  It is tricky to use.  The authors of this book have written a Python module that wraps \li{timeit} into an easy to use Python context.  Please save the following code into an easily accessible file.
