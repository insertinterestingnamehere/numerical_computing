...
and as many rows as there are individuals within our data.
We might also create a table with schema (Name, Employment Status, Income, Education).
There is much flexibility in creating tables. 
...
When you make one or a series of deposits and withdrawals, you are making a transaction.
...
While SQL is not portable across databases in general, we will focus on the parts of SQL that are relatively common.
...
Note that each command we execute returns a another cursor object.  This object is equipped with a method that allows us to look 
at any results of the previous command.  The result is formally known as the \emph{result set}.  
If you use \li{result.fetchall()}, you will see an empty list.  
...
What code is the most frequent code for men between age 25 and 35? For women (between 25 and 35)?  Note that the codes are separated by semicolons.
