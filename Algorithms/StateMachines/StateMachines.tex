...
Regular expressions are especially useful and powerful for find and matching patterns in text.  Many tools can take advantage of the powerful matching features of regular expressions.  You can think of regular expressions as operations on state machines.
For example, the regular expression $(a^*b)^* b^*a$ could be represented as the NFA in \ref{fig:nfa_union}.
\begin{figure}
\begin{tikzpicture}[shorten >=1pt,node distance=2cm,on grid,auto]
\node[state,initial] (q0) {$q_0$};
...
\label{fig:simple_nfa}
\end{figure}
Which is simply a union of the NFAs \ref{fig:nfa1} and \ref{fig:nfa2}.
\begin{figure}
\centering
