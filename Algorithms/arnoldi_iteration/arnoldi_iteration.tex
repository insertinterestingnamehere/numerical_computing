...
\Procedure{lanczos}{$b, Amul, k, tol=1E-8$}
	...
	\State $q_1 \gets \frac{b}{\|b\|_2}$						\Comment{We will use $q_0$ and $q_1$ to store the needed $q_i$}
	...
	\For{$j=0$, $j<k$}									\Comment{Perform the iteration.}
		...
		\EndIf
		\State $z /= \beta_i$
		\State $q_0, q_1 = q_1, z$						\Comment{Store new $q_{i+1}$ and $q_i$ on top of $q_0$ and $q_1$}
	\EndFor
