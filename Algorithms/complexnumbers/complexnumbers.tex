...
The \li{plot_complex} function takes in a function $p$ and bounds and plots it using \li{imshow}.
Figure \ref{fig:funcplot} shows colorplots for $x^2 - 1$ and $x^4-\frac{1}{x^4}$ made using this function.
...
From these plots you can see the poles and zeros. The zeros are the black dots and the poles are the white dots on the plots. the order of the pole or zero can be read from the number of times the full set of colors show up around the pole or zero. As you can see from Figure \ref{fig:funcplot} $x^4-\frac{1}{x^4}$ has eight zeros of order one and a pole of order four. 
...
\begin{problem}
Copy the \li{plot_complex} function from above. We will be using it a lot in this lab.
\begin{itemize}
...
\end{itemize}
Look at the plots. Write a function that prints out an estimate of the roots and/or poles and their order.
\end{problem}
