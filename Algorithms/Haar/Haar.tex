...
to extending the one-dimensional Fourier transform to two-dimensional images.

Given an input matrix of size $2^n \times 2^n$, after first operating on the rows, 
we have two matrices of size $2^n \times 2^{n-1}$,
...
\end{lstlisting}
At this point, the variables \li{LL, LH, HL, HH} contain the current level of coefficients.
