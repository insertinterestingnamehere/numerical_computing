...
\begin{align}
    ...
    \mathbb{E}x_1)'\right] = \left[I_{r^2} - (F \otimes F)\right]^{-1} \cdot
    \text{vec}(Q)
\end{align}
...
def arma_forecast(time_series, phis=array([]), thetas=array([]), mu=0.,
    ...
    """
    Return forecasts for a time series modelled with the given ARMA model.
...
For example,
\begin{lstlisting}
>>> arma_forecast(box_a, phis, thetas, mu, sigma, future_periods=4)
