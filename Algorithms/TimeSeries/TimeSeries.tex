...
Let $\Theta = \{\phi_i, \theta_j, \mu, \sigma_a^2\}$ be the set of paramters
...
characterized by its mean and covariance matrix. But these two quantities are
easily found via the Kalman filter, namely
\begin{align}
    \text{mean} & \quad H\hat{x}_{t|t-1} + \mu \\
    \text{covariance} & \quad HP_{t|t-1}H'
\end{align}
...
examing the estimated autocorrelation and partial-autocorrelation functions of
the data. We will choose $p$ and $q$ that minimize the Akaike information
...
\begin{problem}
    \label{prob:arma:mle}
    Write a function that accepts a time series $\{z_t\}$ and values {\tt
    max\_p}, {\tt max\_q}, and returns the parameters of the model that
    minimizes the AICc, given the constrant that $p < \text{{\tt max\_p}}$, $q
    < \text{{\tt max\_q}}$.
\end{problem}
