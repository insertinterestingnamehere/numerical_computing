...
A matrix with few zero entries is \emph{dense}. 
The SciPy module \li{sparse} has methods to reduce the temporal and spatial complexity of handling sparse matrices. 
To reduce spatial complexity, it has matrix storage methods that are optimized for sparse matrices. 
When we are not using these special methods, we say we are storing the \emph{full} matrix. 
We can also use \li{sparse} methods on dense matrices, but doing so will take longer than using the usual methods for handling full matrices. 
So, sparse versus dense refers to a property of abstract matrices, and sparse versus full refers to how any matrix is stored in memory, regardless of its abstract properties. 
...
As listed in Table \ref{smr}, SciPy has 7 different sparse matrix types, corresponding to 7 ways of storing sparse matrices.
...
Note that we can convert any full matrix to a \li{sparse} matrix of any of the types listed in Table \ref{smr}.
...
The most efficient sparse matrix type for pre-allocation is \li{lil}. 
Once you are done constructing the sparse matrix, you should convert it to a form that is more efficient for computations (like \li{csr} or \li{csc}).
...
Read the documentation to figure out what you should set the keyword parameter \li{k} to be. 
Also, read the documentation to understand the output of this function.
