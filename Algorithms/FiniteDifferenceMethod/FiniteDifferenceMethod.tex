\lab{Algorithms}{The Finite Difference Method}{The Finite Difference Method}
...
\end{bmatrix} \cdot \begin{bmatrix}U_{-1}\\U_0\\ \vdots \\U_{N-2} \\U_{N-1}\end{bmatrix} = \begin{bmatrix}f(x_0) -\alpha(\epsilon + h/2)/h^2 \\f(x_1)\\ \vdots \\ f(x_{N-3})\\ f(x_{N-2})-\beta(\epsilon - h/2)/h^2 \end{bmatrix}
\]
...
\begin{problem}
Use the code given above to solve the singularly perturbed BVP \eqref{eqn:singular_perturbed_BVP} with $\epsilon = 1/10$, $f(x) = -1$.
