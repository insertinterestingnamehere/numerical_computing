...
\begin{algorithm}
\begin{algorithmic}[1]
\Procedure{mindist_simple}{$Y, metric$}
	\State $X \gets Y \text{ sorted along first coordinate}		\Comment{Make sorted copy of $Y$ to perform linesweep.}
	\State $r \gets metric\left(X\left[0\right], X\left[1\right]\right)	\Comment{Initialize the smallest distance as the distance between the first points.}
	\State $l \gets 0$								\Comment{Use indices to track which points are active.}
	\For{$i=2$, $j<k$}								\Comment{Iterate over the remaining points.}
		\While{X[low,0] + r < X[i,0]}					\Comment{Update $l$ to not include points that are too far away.}
			\State $l += 1$
		\EndWhile
		\If{$l < i$}								\Comment{If there are any points to process, process them.}
