...
In this case, after adding in the shortest edges from the currently processed points, the dictionary becomes \li{\{B:(C, B, 5), E:(C, E, 1), F:(C, F, 5)\}}.
...
Since \li{E} no longer needs to be processed, we can remove it from consideration, so we modify this dictionary again so that it is \li{\{B:(C, B, 5), F:(E, F, 4)\}}.
...
\item Get the first edge to add (any edge will do, the shortest edge is a good pick).
...
\begin{problem}
Write a Python function that uses Prim's algorithm to find the minimum spanning tree when it is given a list of edges representing a graph.
