...
Recall that a matrix $Q$ is \emph{unitary} if $Q^\textsf(H} Q = I$ or for real matrices, 
...
Note that if $Q$ is unitary, $Q^{-1} = Q^\textsf(H}$ and $Q^\textsf(H}$ is also unitary, so $\kappa(Q) = \norm{Q} \norm{Q^\textsf(H}} = 1$.
...
This can be written $Px = x - 2v(v^\textsf(H} x)$, so $P$ has matrix representation $P = I - 2v v^\textsf(H}$.
Note that $P^\textsf(H} P = I$; thus $P$ is orthonormal.
...
For example, to find $Q_1$, we choose the right hyperplane to reflect $x$ into the span of $e_1$.
...
This means that $\left( I - 2 v v^\textsf(H} \right) x = \pm \norm{x} e_1$, or, in other words,
...
\[\norm{x}^2 \pm \norm{x} x_1 = u^\textsf(H} x \]

So we have

\begin{align*}
2 v v^\textsf(H} x &= 2 u \frac{\norm{x}^2 \pm x_1 \norm{x}}{\norm{u}^2} \\
		&= 2 u \frac{u^\textsf(H} x}{\norm{u}^2} \\
		&= 2 \frac{u}{\norm{u}} \left( \frac{u}{\norm{u}} \right)^\textsf(H} x
\end{align*}
...
\BEGIN
   ...
   v_k = v_k / \norm{v_k} \\
   R[k:,k:] -= 2 v_k \left( v_k^\textsf(H} R[k:,k:] \right) \\
   Q[k:] -= 2 v_k \left( v_k^\textsf(H} Q[k:] \right) \\
\END \\
\text{return } Q^\textsf(H} , R
\end{pseudo}
...
\begin{pmatrix}
A[:k,:k] & A[:k,k:] \\
A[k:,:k] - 2 v_k v_k^\textsf(H} A[k:,:k] & A[k:,k:] - 2 v_k v_k^\textsf(H} A[k:,k:]
\end{pmatrix}
\]
...
=
\begin{pmatrix}
A[:k,:k] & A[:k,k:] - 2  A[:k,k:] v_k v_k^\textsf(H} \\
A[k:,:k] & A[k:,k:] - 2 A[k:,k:] v_k v_k^\textsf(H}
\end{pmatrix}
\]
...
In other words, you could make an identity matrix and then for $k$ such that $n-2 \geq k > -1$ , do $I[k:,k:] -= 2 v_k v_k^\textsf(H}$
...
Another important thing to notice is that, when computing the matrix $v_k \left( v_k^\textsf(H} A[k:,k:] \right)$, an outer product is needed for the outside operation, \emph{not} an inner product.
...
\BEGIN
   ...
   v_k /= \norm{v_k} \\
   H[k+1:,k:] -= 2 v_k \left( v_k^\textsf(H} H[k+1:,k:] \right) \\
   H[:,k+1:] -= 2 \left( H[:,k+1] v \right) v^\textsf(H} \\
   Q[k+1:] -= 2 v_k \left( v_k^\textsf(H} Q[k+1:] \right) \\
\END \\
\text{return } Q^\textsf(H} , R
\end{pseudo}
