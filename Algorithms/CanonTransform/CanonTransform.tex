...
Recall that a matrix $Q$ is \emph{unitary} if $Q^H Q = I$ or for real matrices, 
...
Note that if $Q$ is unitary, $Q^{-1} = Q^H$ and $Q^H$ is also unitary, so $\kappa(Q) = \norm{Q} \norm{Q^H} = 1$.
...
This can be written $Px = x - 2v(v^H x)$, so $P$ has matrix representation $P = I - 2vv^H$.
Note that $P^H P = I$; thus $P$ is orthonormal.
...
This means that $\left( I - 2 v v^H \right) x = \pm \norm{x} e_1$, or, in other words,
...
\BEGIN
   ...
   v_k = v_k / \norm{v_k} \\
   R[k:,k:] -= 2 v_k \left( v_k^H R[k:,k:] \right) \\
   Q[k:] -= 2 v_k \left( v_k^H Q[k:] \right) \\
\END \\
\text{return } Q^H , R
\end{pseudo}
...
\begin{pmatrix}
A[:k,:k] & A[:k,k:] \\
A[k:,:k] - 2 v_k v_k^H A[k:,:k] & A[k:,k:] - 2 v_k v_k^H A[k:,k:]
\end{pmatrix}
\]
...
In other words, you could make an identity matrix and then for $k$ such that $n-2 \geq k > -1$ , do $I[k:,k:] -= 2 v_k v_k^H$
...
Another important thing to notice is that, when computing the matrix $v_k \left( v_k^H A[k:,k:] \right)$, an outer product is needed for the outside operation, \emph{not} an inner product.
...
\BEGIN
   ...
   v_k /= \norm{v_k} \\
   H[k+1:,k:] -= 2 v_k \left( v_k^H H[k+1:,k:] \right) \\
   H[:,k+1:] -= 2 \left( H[:,k+1] v \right) v^H \\
   Q[k+1:] -= 2 v_k \left( v_k^H Q[k+1:] \right) \\
\END \\
\text{return } Q^H , R
\end{pseudo}
