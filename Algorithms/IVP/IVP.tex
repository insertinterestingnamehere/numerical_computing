...
y(0) &=& 0.
\end{eqnarray*}
Use Euler's method, the Midpoint method, and RK4 to approximate the value of the solution at $x = 2$, with a stepsize of $h = 0.2,$ $ 0.1,$ $0.05 $, $0.025,$ and $0.0125.$ Plot the relative error of each approximation (see Figure \ref{ivp:relative_error}).
\end{problem}
...
y'(0) &=& -1,
\end{eqnarray*} 
on the interval $[0,20]$. For $\gamma = 1/2,$ and $\gamma = 1$, plot the solution $y(t)$ and find $y(20)$ accurate to four significant digits. (Check that the relative error is less than $5 \cdot 10^{-5}$.)  How many subintervals do you need?
\end{problem}
