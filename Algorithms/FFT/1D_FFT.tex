...
\begin{align}
	c_k=\langle e^{\frac{2\pi ikx}N}, f(x)\rangle=\frac1N\sum_{x=0}^{N-1}e^{\frac{-2\pi ikx}N}f(x) . \label{transform_coefficients}
\end{align}
...
Note that the scaling scipy uses does not divide by $N$, so you will need to account for that in comparing your results.
\end{problem}
