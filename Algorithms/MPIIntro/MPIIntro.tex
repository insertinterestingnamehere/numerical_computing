...
\label{lab:MPI_Intro}
  ...
  \section*{MPI: the Message Passing Interface}
    ...
    \end{problem}
      ...
      % \begin{problem}
      %   Determine what implementation of MPI is running on the machine you are using. (Note that \li{mpi4py} must be built on top an MPI distrubtion like MPICH or openMPI. What implementation is \li{mpi4py} built on top of on you machine?)
      % \end{problem}


  \section*{Why \emph{MPI for Python} (\li{mpi4py})?}
  ...
  \section*{Introduction to MPI}
    ...
    \end{verbatim}
    instead.)
    % TODO expand here; getting this to work on everyone's computers will be nontrivial
