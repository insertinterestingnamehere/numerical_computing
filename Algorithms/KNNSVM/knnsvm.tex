...
\begin{problem}
Write a $k$-NN class which accepts initial training data and training labels. It should have a method to classify new samples, given a value of $k$. Load the iris dataset from \li{sklearn.datasets}, and separating the data into training and test sets, implement your class. Test your classifier on the test data given different values of $k$. What are the misclassification rates?
\end{problem}
...
Given a training set of size \emph{n\_samples} and a kernel $k$, with data $X$ and target $Y$, we can use \li{cvxopt} to solve this quadratic programming problem:
\begin{lstlisting}
: import cvxopt
: import scipy as sp
:
