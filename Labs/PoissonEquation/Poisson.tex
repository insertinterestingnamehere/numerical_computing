...
The scalar potential defined over space is a measure of the potential energy a particle would have if placed at that point.
It could be electric potential energy, gravitational potential energy, or anything else governed by a conservative force.

Whenever we have a conservative force we can use a few identities from vector calculus to obtain Poisson's equation for its scalar potential.
...
\[\nabla \times \bold{E} = -\frac{\partial \bold{B} } {\partial t}, \qquad
\nabla \cdot \bold{E} = \frac{\rho}{\epsilon_0},\]
where $\bold{E}$ is the electric field, $\bold{B}$ is the magnetic field, $\rho$ the charge density, and $\epsilon_0$ the permittivity of free space, a constant. Recall $\nabla \times$ and $\nabla \cdot$ represent, respectively, the curl and the divergence operators. In the absence of a changing magnetic field the curl is zero and so we have a conservative force (Note that $\bold{E}$ is not actually a force, but a force per charge. However, we can still treat it as a conservative field proportional to a conservative force). Because the curl is zero we can immediately assume the other properties of a conservative force and write $\bold{E}$ as a gradient,
\[\bold{E}  = - \nabla V = 0.\]
$V$ is called the electrostatic scalar potential, more commonly known as the voltage.
If we insert this into the equation for the divergence of $E$ we find, after the application of the identity $\nabla \cdot (\nabla\psi) = \Delta \psi$m
\[\nabla \cdot ( - \nabla V) = - \Delta V = \frac {\rho}{\epsilon_0}.\]
With this we have Poisson's equation for electrostatics.
If there are no charges (or the divergence of $E$ is zero), then this reduces to Laplace's equation.
...
This means that in reality $V$ is present on both sides of the equation, usually making the equation non-linear.
...
One simple application of the electric potential is to calculate basic properties of simple molecules, starting from a charge distribution and calculating the electric potential field.
We'll assume that the electrons produce a clouds of charge centered around the atoms, approximated by a Boltzmann Distribution $\rho = q e^{-r}$, where $q$ is the relative charge of the atom and $r$ is the distance from the atom.
...
\[\Delta V = -\rho(x,y)\]
The electric potential is usually defined to go to zero at infinity.
We can approximate this using $V=0$ at the boundary conditions on $[-5,5]\times [-5,5]$.
...
If the molecules are moving slowly (i.e. when the temperature is low) electrostatic forces will dominate molecular interactions.
