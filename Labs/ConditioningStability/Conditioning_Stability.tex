...
\begin{lstlisting}
>>> roots = np.sort(roots)
>>> rootsp = np.sort(rootsp)
>>> la.norm(rootsp-roots)/la.norm(perturb)
70361526.49231146
\end{lstlisting}
Thus, the condition number of this problem is something like $10^6$. 
...
>>> k = la.norm(rootsp-roots, np.inf)/la.norm(perturb, np.inf)
>>> k
29060080.472785536
>>> # Estimate the relative condition number in the infinity norm
>>> k*la.norm(w_coeffs, np.inf)/la.norm(roots, np.inf)
2.0056918463389856e+25
\end{lstlisting}
